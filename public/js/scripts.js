jQuery(function ($) {
  // Utility function to safely call Sliders when available
  function callSlidersWhenReady(callback) {
    if (typeof window.Sliders === "function") {
      window.Sliders();
      if (callback) callback();
    } else {
      setTimeout(function () {
        callSlidersWhenReady(callback);
      }, 50);
    }
  }
        ...
        setTimeout(function () {
          ...
          if ($("#hero").hasClass("has-image")) {
            ...
            if ($("#hero-caption").hasClass("height-title")) {
              const spanLetters = document.querySelectorAll(
                "#hero-caption .hero-title.caption-timeline span"
              );

              gsap.set(spanLetters, { scaleY: 0.3, opacity: 0, y: 0 });
              const shuffledSpans = Array.from(spanLetters).sort(
                () => Math.random() - 0.5
              );
          ...
          if (
            $(
              ".showcase-gallery .slider-fixed-content #slide-inner-caption"
            ).hasClass("height-title")
          ) {
            const spanLetters = document.querySelectorAll(
              "#slide-inner-caption .slide-hero-title.caption-timeline span"
            );
            ...
            gsap.to(
              $(
                ".showcase-gallery .slider-fixed-content .slide-hero-subtitle.caption-timeline span"
              ),
              {
                duration: 0.7,
                y: 0,
          ...
          gsap.to(
            ...
            { duration: 2, opacity: 1, delay: 0.3, ease: Power4.easeOut }
          );
    ...
    if (!$("body").hasClass("disable-ajaxload")) {
      $("#page-nav .page-title").on("click", function () {
        ...
        if ($(".page-nav-caption").hasClass("height-title")) {
          const spanLetters = document.querySelectorAll(
            ".page-nav-caption .next-hero-title.caption-timeline span"
          );
          const shuffledSpans = Array.from(spanLetters).sort(
            () => Math.random() - 0.5
          );
          ...
          gsap.to(
            ".page-nav-caption .next-hero-subtitle.caption-timeline span",
            {
              duration: 0.3,
              y: -30,
              opacity: 0,
    ...
    if (!$("body").hasClass("disable-ajaxload")) {
      ...
      $(".next-ajax-link-project").on("click", function () {
        ...
        if ($(".next-project-caption").hasClass("height-title")) {
          const spanLetters = document.querySelectorAll(
            ".next-project-caption .next-hero-title span"
          );
          const shuffledSpans = Array.from(spanLetters).sort(
            () => Math.random() - 0.5
          );
    ...
    if ($(".overlapping-gallery").length > 0) {
      imagesLoaded("body", function () {
        gsap.utils.toArray(".overlapping-gallery").forEach((pinnedGallery) => {
          const pinnedImages =
            pinnedGallery.querySelectorAll(".overlapping-image");
      ...
      if (!isMobile()) {
        $(".overlapping-gallery .trigger-item")
    ...
    if ($(".showcase-portfolio").length > 0) {
      ...
      gsap.utils.toArray(".img-mask").forEach((thumbAnimation, index) => {
        const curtainAnimation =
          thumbAnimation.querySelectorAll(".curtain-row");
        ...
        gsap.to(curtainAnimation, {
          scrollTrigger: {
            ...
            onEnter: function () {
              ...
              thumbAnimation
                .closest(".slide-inner")
                .classList.add("show-caption");
              if ($("body").hasClass("show-loader")) {
                setTimeout(function () {
                  gsap.to(
                    $(".showcase-portfolio .img-mask.animated .curtain-row"),
                    {
                      duration: 0.7,
                      scaleY: 0,
                      opacity: 0,
        ...
        gsap.utils
          ...
          .forEach((parallaxElement, index) => {
            const parallaxElementChild =
              parallaxElement.querySelector(".slide-inner");
